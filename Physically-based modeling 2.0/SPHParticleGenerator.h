...
#ifndef SPHPARTICLEGENERATOR
#define SPHPARTICLEGENERATOR
...
class SPHParticleGenerator
{
    public:
        ...
        SPHParticleGenerator(void);
        ~SPHParticleGenerator(void);
    
        virtual void generate_particles(const double& h, std::vector<SPHParticle>& particles, std::stack<int>& inactive_particles);
    
        virtual void update_generator_transform(const Vector3& position, const Vector3& normal);
    
        virtual void generate_particles_in_grid(std::vector<SPHParticle>& particles, std::stack<int>& inactive_particles);
